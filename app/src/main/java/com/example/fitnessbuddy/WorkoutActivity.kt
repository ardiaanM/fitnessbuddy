...
import com.example.fitnessbuddy.databinding.ActivityWorkoutBinding
...
/* parceable needed for passing objects between classes. needed here to pass it into the workout collection of exercises so it can be shown
for example im not showing the description here in this acticity_workout but i want to show it on the other activity i have to pass it to the other
activity. */
...
class ExercisesMenu : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //spinner to choose different body group.
        bodyPartSpinner.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
            ...
            override fun onNothingSelected(parent: AdapterView<*>?) { // legs is always selected first. needs to be here bcs of "object" error otherwise.
            }
        }
        ...
        sendButton.setOnClickListener {
            if (selectedExercises.isEmpty()) {
                Toast.makeText(this, "Please choose an exercise", Toast.LENGTH_SHORT).show() //toast if collectorCointainer is empty.
            } else {
                WorkoutCollectionOfExercises.startActivity(this, selectedExercises) //else go to the other activity with the selected exercises.
            }
        }


        //clear the CollectorContainer.
    ...
    //grabs workout_image_and_name_for_workoutactivity layout
    private fun displayExercises(exercises: List<Exercise>) {
        exerciseContainer.removeAllViews()

        exercises.forEach { exercise ->
            val exerciseView = layoutInflater.inflate(R.layout.workout_image_and_name_for_workoutactivity, null, false).apply {
                tag = exercise
            }
    ...
    private fun addExerciseToCollector(exercise: Exercise) {
        ...
        val exerciseCollectorView = layoutInflater.inflate(R.layout.workout_spinner_name_collector, collectorContainer, false)
        val exerciseNameTextView: TextView = exerciseCollectorView.findViewById(R.id.workoutSpinnerNameCollectorTV)
        exerciseNameTextView.text = exercise.name

        collectorContainer.addView(exerciseCollectorView)
    }

    //click on exercise again function to remove exercise (there is a clear button) checks if there is the same exercise in the container that u click.
    private fun removeExerciseFromCollector(exercise: Exercise) {
        ...
        for (i in 0 until collectorContainer.childCount) {
            ...
            val exerciseNameTextView: TextView = collectorItemView.findViewById(R.id.workoutSpinnerNameCollectorTV)
